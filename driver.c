...
main(int argc, char **argv)
{
    ...
    extern void _start(), __cleanup();
    startWork((u_long)_start, (u_long)__cleanup);	// addrs not exact, chosen based on "nm | sort -n" output.
    ...
    if(parseOk == 0 && filterCalls(filtCalls, nFiltCalls, recs, nrecs)) {
        ...
        startWork(0xffffffff80000000L, 0xffffffffffffffffL);
        if(noSyscall) {
            x = 0;
        } else {
