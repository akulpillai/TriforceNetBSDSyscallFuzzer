...
{
#ifndef USE_INDIR
    return __syscall(x->nr, x->args[0], x->args[1], x->args[2], x->args[3], x->args[4], x->args[5], x->args[6]);
#else
    u64 ret;
    register u64 nr asm("rax") = x->nr;
    register u64 a0 asm("rdi") = x->args[0];
    register u64 a1 asm("rsi") = x->args[1];
    register u64 a2 asm("rdx") = x->args[2];
    register u64 a3 asm("r10") = x->args[3];
    register u64 a4 asm("r8") = x->args[4];
    register u64 a5 asm("r9") = x->args[5];
    register u64 a6 asm("rcx") = x->args[6]; // pushed onto stack
    asm("pushq %%rcx\n"
        "pushq %%rcx\n"
        "syscall\n"
        "jnb 1f\n"
        ...
        "popq %%rcx\n"
        : "=a"(ret)
        : "a"(nr), "D"(a0), "S"(a1), "d"(a2), "r"(a3), "r"(a4), "r"(a5), "r"(a6), "c"(a6)
        );
    return ret;
#endif
}
